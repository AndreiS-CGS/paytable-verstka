...
- **Имя спрайта выводится из имени файла одним правилом, общим с пайплайном** (`sprite_name()` в
  `pack_atlas.py` и в `paytable_from_confluence.py` — правило продублировано, менять только вместе):
  пробел и `&` → `_`, `+` → `PLUS`, всё в верхний регистр. Имя файла как есть **не** используется.

  Раньше бралось `os.path.splitext(f)[0].upper()`, то есть что художник напечатал, то и становилось
  именем. Совпадало это с токенами GDD случайно, пока не разошлось: один атлас содержал одновременно
  `DIAMOND_SIGNBOARD` и `2_WILD&SIGNBOARD`, а текст просил `DIAMOND&ACE` — 27 имён не разрезолвились.
  Ошибки при этом нет нигде: TMP молча подставляет запасной глиф.
...
Укладка рядами («полками»), **first-fit decreasing**: сначала самые широкие, и каждый спрайт идёт в
первую полку, куда ещё влезает, а не только в текущую. При одинаковой высоте это на практике даёт
теоретический минимум числа полок. Более умные упаковщики (MaxRects и родственники) на однородном
входе отыгрывают единицы процентов и не стоят усложнения.

> Порядок важен, и это не теория. Раньше укладка шла **в порядке имён файлов** и закрывала полку,
> как только следующий спрайт не влезал. Баннер шириной 1423 при остатке 500 обрывал ряд — в одном
> реальном атласе две полки держали 548 и 840 пикселей при ширине 2011, и уходило 8 полок там, где
> хватает 7. С FFD на том же наборе хвосты стали 39, 30, 20, 4, 42, 12 и остаток в последней.

`--pow2` включён **по умолчанию**: высота получает собственную степень двойки. Ряды однородные,
контент обычно намного шире, чем выше, и квадрат потратил бы половину текстуры. Отключается через
`--no-pow2`, если зачем-то нужен ровно `--atlas-size`.

> Без `--pow2` атлас — квадрат размером `--atlas-size` независимо от содержимого, и нигде об этом не
> говорится. Один отгруженный атлас вышел 2048×2048 при 1052 пикселях рядов: половина текстуры
> впустую, ни ошибки, ни предупреждения. Поэтому флаг и стал поведением по умолчанию.
...
| Класс | Высота в референсе | R из GDD (в cap) | **P при кегле 32** |
|---|---|---|---|
| Арт символа (обычный инлайн) | 48–60px | **5.0×** | **340%** |
| Бейдж/лого джекпота (широкий, плоский) | 24px | **2.2×** | **150%** |
...
```
P = 100 × R × capLine_em / 1.5
```

где `R` — соотношение, снятое с рендера GDD (высота спрайта в высотах прописной), а `capLine_em` —
высота прописной в em у того шрифта, которым набран текст (см. «Вертикальная поправка»).
При `capLine_em ≈ 1.02`: `R = 5` → `P ≈ 340`, `R = 2.2` → `P ≈ 150`.

**Деление в конце обязательно.** Соотношение из GDD не переносится в сборку один к одному:
подстановка `R` напрямую даёт слишком крупно. Делитель **1.5** — подтверждённая на рендерах
величина, не догадка: сначала пробовали 2, но при нём широкие бейджи джекпотов (`GRAND_JACKPOT` —
соотношение около 11:1 после приведения к высоте атласа) сжимались меньше чем до трети натуральной
величины и надпись внутри не читалась.
Мерить `R` по-прежнему надо по GDD, потому что это единственный источник пропорции; делится уже
результат.
...
> это **текущий** кегль: внутри тега `size` он больше базового, и поправка уезжает вместе с `P`.
