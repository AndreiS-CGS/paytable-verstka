...
|---|---|---|---|
| Арт символа (обычный инлайн) | 48–60px | **5.0×** | **250%** |
| Бейдж/лого джекпота (широкий, плоский) | 24px | **2.2×** | **112%** |
...
При `capLine_em ≈ 1.02`: `R = 5` → `P ≈ 250`, `R = 2.2` → `P ≈ 112`.

**Деление на 2 в конце обязательно.** Соотношение из GDD не переносится в сборку один к одному:
подстановка `R` напрямую даёт вдвое крупнее нужного. Половина — подтверждённая на рендере величина,
не догадка. Мерить `R` по-прежнему надо по GDD, потому что это единственный источник пропорции;
делится уже результат.
